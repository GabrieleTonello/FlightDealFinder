...
/**
 * Lambda handler for the Workflow Trigger function. Triggered by SQS Deal Queue messages.
 *
 * <p>Parses DealBatchMessage from each SQS record, starts a Step Functions Matching Workflow
 * execution with the deal data as input, and emits CloudWatch metrics for workflows started and
 * start failures.
 *
 * <p>Throws on StartExecution failure so SQS retries the message.
 */
@Slf4j
public class WorkflowTriggerHandler implements RequestHandler<SQSEvent, Void> {
  ...
  /**
   * No-arg constructor for Lambda runtime. Creates a Guice injector with WorkflowTriggerModule and
    ...
    this.stateMachineArn =
        injector.getInstance(
            com.google.inject.Key.get(
                String.class, com.google.inject.name.Names.named("STATE_MACHINE_ARN")));
  }
  ...
  /**
   * Processes a single SQS record: parses the deal batch message, starts a Step Functions
   * execution, and emits metrics. Throws RuntimeException on failure so SQS retries the message
   * (Req 6.3).
   */
  private void processRecord(SQSEvent.SQSMessage record) {
    ...
    try {
      ...
      String searchTimestamp =
          dealBatch.has("searchTimestamp") ? dealBatch.get("searchTimestamp").asText() : "unknown";
