...
/**
 * Proxy client for querying an external flight API for deals per destination. Designed for
 * constructor injection so it can be mocked in tests.
 */
public class FlightApiClient {
    ...
    try {
      ...
      if (response.statusCode() != 200) {
        throw new FlightApiException(
            destination, "API returned HTTP " + response.statusCode(), "HTTP_ERROR");
      }
  ...
  private List<FlightDeal> parseDeals(String responseBody) throws FlightApiException {
    try {
      List<Map<String, Object>> rawDeals =
          objectMapper.readValue(responseBody, new TypeReference<>() {});

      return rawDeals.stream().map(this::toFlightDeal).toList();
    } catch (Exception e) {
      throw new FlightApiException(
