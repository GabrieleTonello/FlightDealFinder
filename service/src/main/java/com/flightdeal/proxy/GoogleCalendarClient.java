...
/**
 * Proxy client for the Google Calendar FreeBusy API.
 * Authenticates via OAuth2 access token and retrieves free/busy windows for a date range.
...
public class GoogleCalendarClient {
    ...
    /**
     * Retrieves free time windows from Google Calendar for the given date range.
     * Calls the FreeBusy API and inverts the busy periods to compute free windows.
     *
     ...
     */
    public List<TimeWindow> getFreeBusyWindows(String startDate, String endDate) throws CalendarApiException {
    ...
    private List<TimeWindow> computeFreeWindows(List<Map<String, String>> busyPeriods,
        ...
        List<Map<String, String>> sorted = busyPeriods.stream()
        ...
        for (Map<String, String> busy : sorted) {
            LocalDate busyStart = Instant.parse(busy.get("start"))
                    .atZone(ZoneOffset.UTC).toLocalDate();
            LocalDate busyEnd = Instant.parse(busy.get("end"))
                    .atZone(ZoneOffset.UTC).toLocalDate();

            if (currentStart.isBefore(busyStart)) {
                freeWindows.add(TimeWindow.builder()
