 ...
 * from the earliest departure to the latest return across all deals, queries Google Calendar, and
...
public class CalendarService {
  ...
  public List<TimeWindow> lookupFreeWindows(List<FlightDeal> deals) {
    ...
    String earliestDeparture =
        deals.stream().map(FlightDeal::getDepartureDate).min(String::compareTo).orElseThrow();

    String latestReturn =
        deals.stream().map(FlightDeal::getReturnDate).max(String::compareTo).orElseThrow();

    log.info(
        "Looking up calendar free windows from {} to {} for {} deals",
        earliestDeparture,
        latestReturn,
        deals.size());
    ...
    try {
      List<TimeWindow> freeWindows =
          googleCalendarClient.getFreeBusyWindows(earliestDeparture, latestReturn);
