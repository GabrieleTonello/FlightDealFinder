 ...
 *
 * Formats an email body containing destination, price, departure date,
 * return date, and airline for each matched deal, then sends it to the
 * configured recipient. Throws RuntimeException on failure so that
 * Step Functions retry policies can handle transient errors.
 */
@Slf4j
@Singleton
public class NotificationService {
    ...
    public String sendDealNotification(List<FlightDeal> matchedDeals) {
        log.info("Sending deal notification with {} matched deals to {}", matchedDeals.size(), recipientEmail);

        String subject = String.format("Flight Deal Alert: %d matching deal%s found!",
