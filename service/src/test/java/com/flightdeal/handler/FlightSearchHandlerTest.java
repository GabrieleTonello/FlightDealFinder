...
class FlightSearchHandlerTest {
    ...
    private FlightSearchHandler createHandler(List<String> destinations) {
        return new FlightSearchHandler(
                ...
                metricsEmitter, TOPIC_ARN, destinations);
    }

    private FlightDeal deal(String destination, String price, String dep, String ret, String airline) {
    ...
    void handleRequest_successPath_savesAndPublishesDeals() throws Exception {
        List<FlightDeal> parisDeals = List.of(deal("Paris", "299.99", "2025-03-01", "2025-03-08", "AirFrance"));
        List<FlightDeal> tokyoDeals = List.of(deal("Tokyo", "899.00", "2025-04-01", "2025-04-10", "ANA"));
    ...
    void handleRequest_partialFailure_continuesWithSuccessfulDestinations() throws Exception {
        List<FlightDeal> parisDeals = List.of(deal("Paris", "299.99", "2025-03-01", "2025-03-08", "AirFrance"));
        when(flightApiClient.searchDeals("Paris")).thenReturn(parisDeals);
        when(flightApiClient.searchDeals("Tokyo")).thenThrow(new FlightApiException("Tokyo", "API error", "HTTP_ERROR"));
        when(snsClient.publish(any(PublishRequest.class))).thenReturn(PublishResponse.builder().build());
    ...
    void handleRequest_allDestinationsFail_noSaveNoPublish() throws Exception {
        when(flightApiClient.searchDeals("Paris")).thenThrow(new FlightApiException("Paris", "timeout", "TIMEOUT"));
        when(flightApiClient.searchDeals("Tokyo")).thenThrow(new FlightApiException("Tokyo", "error", "HTTP_ERROR"));
    ...
    void handleRequest_emitsCorrectMetrics() throws Exception {
        List<FlightDeal> parisDeals = List.of(
                deal("Paris", "299.99", "2025-03-01", "2025-03-08", "AirFrance"),
                deal("Paris", "350.00", "2025-03-15", "2025-03-22", "Delta"));
        List<FlightDeal> tokyoDeals = List.of(
                deal("Tokyo", "899.00", "2025-04-01", "2025-04-10", "ANA"));
    ...
    void handleRequest_emitsMetricsEvenWhenAllFail() throws Exception {
        when(flightApiClient.searchDeals("Paris")).thenThrow(new FlightApiException("Paris", "err", "HTTP_ERROR"));
        when(flightApiClient.searchDeals("Tokyo")).thenThrow(new FlightApiException("Tokyo", "err", "TIMEOUT"));
