...
class WorkflowTriggerHandlerTest {
  ...
  private static final String STATE_MACHINE_ARN =
      "arn:aws:states:us-east-1:123456789:stateMachine:MatchingWorkflow";
  ...
  void setUp() {
    handler =
        new WorkflowTriggerHandler(sfnClient, metricsEmitter, objectMapper, STATE_MACHINE_ARN);
  }
    ...
    when(sfnClient.startExecution(any(StartExecutionRequest.class)))
        .thenReturn(
            StartExecutionResponse.builder()
                .executionArn(
                    "arn:aws:states:us-east-1:123456789:execution:MatchingWorkflow:deal-workflow-123")
                .build());
  ...
  void handleRequest_successfulStart_emitsWorkflowsStartedMetric() {
    ...
    when(sfnClient.startExecution(any(StartExecutionRequest.class)))
        .thenReturn(StartExecutionResponse.builder().executionArn("arn:exec").build());
  ...
  void handleRequest_multipleSqsRecords_startsWorkflowForEach() {
    String body1 =
        "{\"deals\":[],\"searchTimestamp\":\"2025-01-15T10:00:00Z\",\"destinationsSearched\":0}";
