...
/**
 * Property 1: For any set of configured destinations, when the Flight Search Lambda is invoked, the
 * number of flightApiClient.searchDeals() calls equals the number of destinations.
 *
...
class AllDestinationsQueriedPropertyTest {
  ...
  @Property(tries = 100)
  void allConfiguredDestinationsAreQueried(@ForAll("destinations") List<String> destinations)
      throws Exception {
    ...
    when(flightApiClient.searchDeals(anyString()))
        .thenReturn(
            List.of(
                FlightDeal.builder()
