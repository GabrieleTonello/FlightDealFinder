...
/**
 * Property 8: For any non-empty list of deals, the CalendarService queries with min(departureDate)
 * to max(returnDate).
 *
...
class CalendarDateRangePropertyTest {
  ...
  @Property(tries = 100)
  void calendarQuerySpansMinDepartureToMaxReturn(@ForAll("dealList") List<DealDates> dealDates)
    ...
    String expectedStart =
        dealDates.stream().map(dd -> dd.departure).min(String::compareTo).orElseThrow();
    String expectedEnd =
        dealDates.stream().map(dd -> dd.returnDate).max(String::compareTo).orElseThrow();
    ...
    assertEquals(
        expectedStart, startCaptor.getValue(), "Calendar query start should be min departure date");
    assertEquals(expectedEnd, endCaptor.getValue(), "Calendar query end should be max return date");
  }

  @Provide
  Arbitrary<List<DealDates>> dealList() {
    Arbitrary<DealDates> dealArb =
