...
/**
 * Property 10: For any FlightDeal and TimeWindow, the deal matches iff
 * departure >= window.start AND return <= window.end.
 *
...
class MatchingPredicatePropertyTest {
    ...
    @Property(tries = 100)
    void dealMatchesIffEntirelyWithinWindow(
            @ForAll("dealAndWindow") DealAndWindow input) {
        ...
        if (shouldMatch) {
            assertFalse(result.isEmpty(), "Deal should match: departure=" + input.departureDate
                    + " >= windowStart=" + input.windowStart + " AND return=" + input.returnDate
                    + " <= windowEnd=" + input.windowEnd);
        } else {
            assertTrue(result.isEmpty(), "Deal should NOT match: departure=" + input.departureDate
                    + " vs windowStart=" + input.windowStart + ", return=" + input.returnDate
                    + " vs windowEnd=" + input.windowEnd);
        }
    }

    @Provide
    Arbitrary<DealAndWindow> dealAndWindow() {
        return Arbitraries.integers().between(0, 300).flatMap(windowStartOffset ->
                Arbitraries.integers().between(5, 60).flatMap(windowLength ->
                        Arbitraries.integers().between(-30, 90).flatMap(depRelativeToWindow ->
                                Arbitraries.integers().between(3, 40).map(tripLength -> {
                                    LocalDate base = LocalDate.of(2025, 1, 1);
                                    LocalDate wStart = base.plusDays(windowStartOffset);
                                    LocalDate wEnd = wStart.plusDays(windowLength);
                                    LocalDate dep = wStart.plusDays(depRelativeToWindow);
    ...
    record DealAndWindow(String departureDate, String returnDate,
