...
/**
 * Property 6: For any non-empty list of deals stored, the SNS message contains
 * exactly those deals. Capture the SNS PublishRequest and verify the message body
 * contains all deal destinations.
 *
...
class PublishAfterStoragePropertyTest {
    ...
    @Property(tries = 100)
    void publishedDealsMatchStoredDeals(@ForAll("dealList") List<DealInput> dealInputs) throws Exception {
        ...
        for (FlightDeal deal : deals) {
            assertTrue(messageBody.contains(deal.getDestination()),
    ...
    Arbitrary<List<DealInput>> dealList() {
        Arbitrary<DealInput> dealArb = Arbitraries.of("Paris", "Tokyo", "London", "Berlin")
                .flatMap(dest -> Arbitraries.bigDecimals()
                        .between(new BigDecimal("50.00"), new BigDecimal("2000.00"))
                        .ofScale(2)
                        .flatMap(price -> Arbitraries.of("AirFrance", "Delta", "ANA", "Lufthansa")
